...
public class KdTree {
	...
	Vector<AgentTreeNode> agentTree_=new Vector<AgentTreeNode>();
	
	/**
	 * \brief   finalructs a <i>k</i>d-tree instance.
	 * \param   sim  The simulator instance.
	 */
	public KdTree(RVOSimulator rvoSimulator) {
	...
	void buildAgentTreeRecursive(int begin, int end, int node){
		...
		if (end - begin > RVO_MAX_LEAF_SIZE) {
			...
			final float splitValue = (float) (0.5f * (agentTree_.get(node).maxCoord.getElement(coord) + agentTree_.get(node).minCoord.getElement(coord)));
	...
	void computeAgentNeighbors(Agent agent, float rangeSq){
		MutableFloat mutableRangeSq = new MutableFloat(rangeSq);
		queryAgentTreeRecursive(agent, mutableRangeSq, 0);
	}


	void queryAgentTreeRecursive(Agent agent, MutableFloat rangeSq, int node){
